...
export const SelectorButton: React.FC<SelectorButtonProps> = ({ title, selected, onClick }) => {
	return (
		<div className="w-1/2 max-md:w-full relative" aria-label="Selector button">
			<button className={`w-full cursor-pointer py-5 max-md:py-3 max-md:px-16 px-40 rounded-lg text-lg font-bold text-center relative ${selected ? 'bg-[#8A70FF] text-white' : 'text-black'}`} onClick={onClick} aria-label="Selector button">
				{title}
			</button>
			{selected && (
				<div
